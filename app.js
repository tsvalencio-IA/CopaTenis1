...
function buildAlbumMiniCard(vendor, index) {
  ...
  return `
    <article class="album-mini-card ${team}">
      <div class="album-mini-top">
        <span class="album-mini-no">#${getStickerNumber(index)}</span>
        <span class="album-mini-team">${escapeHtml(teamName(team))}</span>
      </div>
      <div class="album-mini-photo">${photo}</div>
      <div class="album-mini-bottom">
        <strong>${escapeHtml((vendor.shortName || vendor.name || 'Vendedor').toUpperCase())}</strong>
        <span>${escapeHtml(vendor.position || 'Vendedor')}</span>
      </div>
    </article>
  `;
}
